 ...
 *
 * @remarks
 * Returned by the {@link ReadableTren.getFees | getFees()} function.
 *
 * @public
 */
export class Fees {
   ...
   * the `when` parameter.
   *
   * To calculate the borrowing fee in TrenUSD, multiply the borrowed TrenUSD amount by the borrowing rate.
   *
   * @example
   * ```typescript
   * const fees = await tren.getFees();
   *
   * const borrowedTrenUSDAmount = 100;
   * const borrowingRate = fees.borrowingRate();
   * const borrowingFeeTrenUSD = borrowingRate.mul(borrowedTrenUSDAmount);
   * ```
   */
   ...
   *
   * @param redeemedFractionOfSupply - The amount of TrenUSD being redeemed divided by the total supply.
   ...
   * precise, it depends on the fraction of the redeemed amount compared to the total TrenUSD supply,
   * which must be passed as a parameter.
   *
   * To calculate the redemption fee in TrenUSD, multiply the redeemed TrenUSD amount with the redemption
   * rate.
   *
   * @example
   * ```typescript
   ...
   *
   * const redeemedTrenUSDAmount = Decimal.from(100);
   * const redeemedFractionOfSupply = redeemedTrenUSDAmount.div(total.debt);
   * const redemptionRate = fees.redemptionRate(redeemedFractionOfSupply);
   * const redemptionFeeTrenUSD = redemptionRate.mul(redeemedTrenUSDAmount);
   * ```
   */
