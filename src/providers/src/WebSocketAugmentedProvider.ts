...
export const WebSocketAugmented = <T extends new (...args: any[]) => BaseProvider>(Base: T) => {
  const webSocketAugmentedProvider = class extends Base implements WebSocketAugmentedProvider {
    ...
    on(eventName: EventType, listener: Listener) {
      ...
      } else if ((eventName as any) === "block") {
    ...
    off(eventName: EventType, listener: Listener) {
      if (isTransactionHash(eventName) || (eventName as any) === "block") {
